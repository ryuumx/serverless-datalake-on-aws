...
* [Configuring Interface Endpoints for Athena](https://docs.aws.amazon.com/vpc/latest/userguide/vpce-interface.html#create-interface-endpoint)

* Configure an RDS connection for Glue
    * Create a VPC. You can use the provided [CloudFormation template](./cloudformation_vpc.yaml) or use the following steps:
        * Create a set of Private Subnets
        * Create a NAT Gateway
	* Create a Route Table with `0.0.0.0/0` pointing to NAT Gateway. Associate RT to Private Subnets
    * Create a VPC Endpoint for s3. Associate the earlier RT to this Endpoint.
    * Create an RDS Cluster. Avoid using Easy Create - use Standard Create to create an initial database `glue_db`
    * After the RDS writer is *created*, go to Glue console and *Add connection*
    * Enter the relevant values. :ballot_box_with_check: on Require SSL if needed.
    * Fill in the relevant DB auth values, using the earlier database name `glue_db`
    * Complete the form and *Test Connection*

* Create a Glue Job to import `reference_data` in s3 to a SQL table in RDS Aurora
    * Finish Extra Materials #2
    * In the Glue console left sidebar, click on **Jobs**.
    * Click **Add job**. 
        * Name: *Import reference_data to RDS*
	* IAM role: *GlueServiceRoleLab*
	* Type and Glue version: *Leave as default*
	* This job runs: *A proposed script generated by AWS Glue*. Click **Next**
	* Choose a data source: *reference_data*
	* Choose a transform type: *Change schema*
	* Choose a data target: *Create tables in your data target*, Connection: *Aurora cluster name*, Database name: *glue_db*
	* Map schema and **Save job and edit script**
	* In the Edit Script view, click **Run job**
		
    * Test configuration e.g. from a bastion host. You should see the database `glue_db` with a table for `reference_data`

* Analyze S3 data directly on Athena without using Glue tables.
    * Amazon provides publicly-accessible large datasets via [registry.opendata.aws](https://registry.opendata.aws). These datasets include census data, satelite data, healthcare, social platforms etc.
    * Try importing a dataset of customer reviews from amazon.com using the `amazon_reviews_parquet` S3 bucket.
    * Run the following command in Athena
    ```
    ...
    ```
    * Run the repair command to update the metastore for new data:
    ```
			MSCK REPAIR TABLE amazon_reviews_parquet;
    ```
		
    * Create a View for Books from the data: 
    ```
    ...
    ```
    * You can now preview a list of Reviews for Books sold on amazon.com
